...
def run_extraction(batch_size: int = 20, model: str = DEFAULT_MODEL, dry_run: bool = False,
    ...
    docs = supabase_client.get_documents_by_status(
        ...
        exclude_buyers=exclude_buyers, include_url_like=include_url_like)
    if not docs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exclude-buyers", default=None,
                        help="comma-separated documents.buyer_name values to EXCLUDE "
                             ...
                             "operator-approved)")
    parser.add_argument("--include-hosts", default=None,
                        help="comma-separated url host substrings to RESTRICT this "
                             "run to (the targeted-drain scope, e.g. "
                             "tpsb.ca,peelpoliceboard.ca)")
    ...
    include_url_like = ([h.strip() for h in args.include_hosts.split(",") if h.strip()]
                        if args.include_hosts else None)
