...
## Da-Ré Advisory firewall (recorded 2026-07-21, for counsel)

A second seam, distinct from the investor seam: Da-Ré Advisory plans a
...
- **Direction of information flow.** Signal North to Da-Ré Advisory only, and
  only as to any subscriber: Da-Ré Advisory consumes SN's published outputs on
  the same terms as everyone else, no privileged feed, no early access,
  and the SN feed is one intelligence input among several on the Da-Ré Advisory
  side. Nothing flows back. Client uploads (financials, capability
  decks, past proposals, CVs) live in a per-client, per-engagement
  knowledge base on the Da-Ré Advisory side; no client RFP response, draft,
...
- **IP separation.** The drafting engine reuses SN's audit-loop
  ...
  calibration-audit discipline) but is built as separate Da-Ré Advisory IP with
  zero SN code, zero SN data, zero SN branding.
- **Conflict-check protocol.** Before any drafting engagement, Da-Ré Advisory
  ...
  live on the Da-Ré Advisory side; SN never sees who Da-Ré Advisory drafts for.
- **Founding-member cross-entity benefit (counsel question for
  September).** SN founding membership includes preferred standing at
  Da-Ré Advisory (priority engine access, founding engagement rates) as a
  cross-entity perk, never a bundled product: separate contracts,
  separate invoices, the SN membership agreement referencing the Da-Ré Advisory
  benefit while Da-Ré Advisory engagements remain their own paper. For counsel:
